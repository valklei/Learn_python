...
ALTER TABLE orders 
ADD  discounter_price decimal(10,2) default 0;

update orders 
set discounter_price = price_1 * 0.9;
...
order by discounter_price desc;
 ...
 select *,
 case 
			when discounter_price < 100.00 then 'LOW'
			when discounter_price >= 100 and discounter_price <= 400 then 'MIDDLE'
            else 'HIGH'
END as status_1
 FROM orders;
